...
export async function POST(req: Request) {
  ...
  if (session.papel !== "TECNICO") return new NextResponse("Apenas técnicos enviam fotos", { status: 403 });
  ...
  const relatorio = await prisma.relatorio.findUnique({ where: { uuid: relatorioUuid } });
  if (!relatorio || relatorio.tecnicoId !== session.userId)
