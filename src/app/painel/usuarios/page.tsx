...
export default async function Usuarios({ searchParams }: { searchParams: Promise<{ erro?: string }> }) {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-bold tracking-tight text-marinho">Usuários</h1>
      ...
      <form action={criarUsuario} className="cartao grid gap-2 p-3 sm:grid-cols-[2fr_2fr_1fr_1fr_auto]">
        <input type="text" name="nome" required placeholder="Nome" className="campo-input p-2 text-sm" />
        <input type="email" name="email" required placeholder="E-mail" className="campo-input p-2 text-sm" />
        <input type="password" name="senha" required minLength={8} placeholder="Senha" className="campo-input p-2 text-sm" />
        <select name="papel" className="campo-input p-2 text-sm">
          ...
          <tbody>
            {usuarios.map((u) => (
              <tr key={u.id} className="border-b border-slate-100 last:border-0">
                ...
                <td className="p-3">{u.ativo ? "Ativo" : <span className="chip bg-alerta-bg text-alerta">Inativo</span>}</td>
                <td className="p-3">
                  {u.id !== session.userId && (
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <form action={redefinirSenha} className="flex items-center gap-1">
                        ...
                        <input
                          ...
                          className="campo-input w-28 p-1 text-xs"
                        />
